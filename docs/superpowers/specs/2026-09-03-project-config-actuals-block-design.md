...
> **Correction (2026-09, found during Task 4's live verification):** the "already the final id in both cases" claim above is wrong for the create-new-project flow. `BLANK_PROJECT()` (`project-config.html:373`) initializes `id: ''` — an empty string, never a client-generated UUID — and `_pushProjectToApi()`'s own guard (`if (!project?.id) return;`, `js/api-sync.js:241`) means a new project's `id` is *never* populated and `Api.projects.create()` is *never* called: creating a new project via this page's Save button has never persisted anything server-side. This predates this branch and is unrelated to the redirect change itself (harmless there — an empty `?projectId=` falls through to `portfolio.html`'s list view). Logged as a critical, unfixed pre-existing bug; see the cycle's finish-cycle report for details. The redirect's edit-existing-project path is correct and was verified live.
