...
**Date:** 2026-07-02
**Context:** the user has manually run the same closeout sequence — test, optional manual verification, code review, merge, doc sync, report — across multiple prior cycles (both code development and documentation audits, e.g. the PRD.md audit merged earlier today on branch `main` directly, and prior feature branches). This spec codifies that sequence into a single explicit command so it stops being re-derived by hand each time.
