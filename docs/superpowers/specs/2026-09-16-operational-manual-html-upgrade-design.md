...
- **Burndown chart** — previously only confirmed to exist in `PRD.md`, never explained. It's the one chart on the single-project dashboard: a filterable (single task or whole project; monthly or weekly) line chart whose solid line is actual consumption ("Remaining Hours" if the project has a budget, "Cumulative Hours" if not), against up to two dashed reference lines — the phasing-derived estimated budget and/or the planning-derived estimated hours — so a reader can see at a glance whether real consumption is running ahead of or behind the plan. Together with the KPI cards, Monthly Summary Table, and the three Summary-by breakdowns (all already covered above), this is the complete "information and reports" surface of the single-project dashboard the user asked to have explained as a whole. (Source: `PRD.md` §6.1, added alongside this spec after verifying the chart's actual datasets in `portfolio.html` — previously only a one-line mention that the chart exists.)
...
Per explicit user request, this needs the same "walk through it properly, don't just list facts" treatment as Resource Planning (§5) and the task-cost formula (§4) — the concise pass mentions actuals upload in passing under Project Configuration (§7) but never explains the mechanism itself well enough to trust it. Structure as:

1. **Where you upload from.** Two entry points that are the *same* underlying action, not two different features: the project's own configuration form (§7) and the Project Reporting single-project detail page (§6) both have a "📂 Load Actuals" button — worth saying explicitly that these are one mechanism reachable from two places, since a reader who's seen it in one place might not realize the other button does the same thing.
...
3. **What happens to each row on upload.** A row with a missing/blank D365 Project ID is skipped entirely; a row that has a project ID but incomplete data otherwise (blank date or hours) is still stored as-is, not skipped. Rows are grouped by project code.
4. **Replace, not append — the single most important behavior to get right.** Uploading a new file for a project code **replaces every previously-imported row for that code**, it does not add to them. A reader who expects "upload = append" (a very natural assumption) will be surprised the first time they re-upload — this needs to be stated plainly, not just implied by word choice.
5. **The rate snapshot.** Each row's hourly rate (Fee) is resolved and locked in once, at the moment of import, by matching task+role against the project's configured resources. A later change to that role's rate does not retroactively change already-imported rows — re-uploading the file is the only way a project's actuals pick up a rate change.
6. **Date parsing.** Unambiguous whenever one of the two date numbers is >12 (can't be a month); falls back to the source export's known MM/DD convention only when genuinely ambiguous (both numbers ≤12); a date that resolves to something that isn't a real calendar date rejects the **entire upload**, naming the offending spreadsheet row — not a partial import of everything else.
7. **Reviewing/managing what was uploaded, afterward.** Two more previously-under-documented pieces from `PRD.md` §8.4 worth restoring: the admin-only Timesheet Management page's columns (Client/Project/Project code, Uploads, Rows, Last uploaded) are independently sortable by clicking the Client/Project/Project code headers (cycling ascending → descending → unsorted); its pipeline-year selector defaults to the current calendar year if active, otherwise the most recently active year, with an explicit "All years" option — the only place a project code with no linked cost-grid version (so no pipeline year of its own) is shown at all. Downloaded exports (from either this admin page or a single project's own Actuals section, §7) use the same filename pattern: `<Client>_<Project>_<ProjectCode>_<YYYYMMDD>.xlsx`.
...
- **Currencies — an entire admin tab previously missing from `PRD.md` altogether**, not just under-detailed like the other items in this list. EUR is fixed (1:1, always active, not editable); any other currency starts inactive and only appears as a selectable option elsewhere in the app (offer/project currency dropdowns, rate card columns) once an admin activates it here with an initial "1 EUR = X" rate. Once active, its rate can be updated at any time from the same table, each update timestamped; a History button (all currencies except EUR) shows the rate's full change log over time. Worth being explicit that this is the *one* exchange rate the whole app reads for every cross-currency figure — the pipeline board's mixed-currency column totals (already covered above) and the rate card's per-currency columns (also covered above) are not separately configured, they both read this same admin-managed rate. (Source: `PRD.md` §7.7, newly added alongside this spec after discovering the tab in `config.html` — not a refinement of existing text, this tab had zero prior documentation.)
- **Clients, and how per-client cost grids actually work.** The registry itself is simple (name, and at most one client group membership), but the mechanism worth walking through is the **💲 Costgrid** button on each client row — an easy label to misread, since it opens that client's *rate card*, not a cost grid itself. The modal shows every role with two editable columns: **Agency default** (read-only, from the global rate card — or the role's own base rate if there's no global card) and **Client custom** (blank by default, meaning "use the agency default"; entering a value overrides it for this client only). For an active non-EUR currency, an extra column appears the same way, pre-populated with that role's agency default in that currency. Saving creates or updates this client's rate card. **The connection to cost grids**: this is exactly the "client rate card override" step already covered as the first link in the rate fallback chain under the offer editor (§4) — selecting this client on any offer automatically applies whatever custom rates are set here, with no further action needed on the offer itself. One more thing worth flagging explicitly: rate-card management lives **only** here, not in the separate User Administration page — a reader looking for it in the wrong admin screen would not find it.
- **Pipeline years and POTs — walked through as a whole, not as separate fact-lists.**
  1. **What a POT actually is, defined plainly before any mechanics:** a Client POT is the total target revenue — the maximum "wallet potential" — allocated to a specific client (or client group) for one forecasting year: existing recurring business plus identified upsell/cross-sell opportunity, combined into one benchmark figure. Every feature below is a different view onto that one number and how real pipeline is tracking against it. (Definition confirmed directly by the user and added to `PRD.md` §7.4 — this term appeared throughout the document with no definition anywhere, a genuine gap this spec closes rather than guesses at.)
  2. **Pipeline years themselves (View A)** — the admin screen listing every pipeline year with a Visible/Hidden badge; a year can be shown or hidden from the board (§4.7), deleted (blocked if any offer still references it), or drilled into for its POTs. Two export links, Proposal Phasing and Project Phasing, sit here too — per-year monthly budget distribution exports, one for offers and one for active projects.
  3. **Setting up a target (View B, "+ New POT")** — pick what the target applies to: one specific client, a client group, or one of two general-purpose scopes for revenue not yet tied to a named client — "Unassigned / To be Identified" and "New Biz." Enter the amount; the year is whichever pipeline year View B is currently showing.
  4. **Where you see progress toward it.** Three surfaces, all reading the same underlying numbers: the **POT banner** at the top of View B (total target and Committed+Anticipated achievement % across every POT that year), the **5 stage-summary cards** (count and professional-fee value per pipeline stage, for the whole year — not scoped to one POT), and — scoped to one specific POT — the **View Details modal**'s 4 KPI cards (Target, Total Committed+Anticipated, Committed, Anticipated, each as a % of that POT's own target) plus a **Proposals** list of every offer counted toward it (Canceled included, Draft excluded, each linking straight to that offer). The same progress is also visible from the Pipeline board's own detail panel (§4.8) for any offer linked to a client/group with a POT — worth stating it's the same number in both places, not two separately tracked figures.
  5. **History.** Every edit to a POT's amount is logged — date, who made the change, old value, new value — viewable as a simple newest-first change log from the View Details modal. This is what lets someone answer "what was this target before, and when did it change," not just "what is it now."
