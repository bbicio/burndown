...
- **Description:** Derive rounds to a 0.1 grid, but the planning-grid renderer immediately re-snaps every cell to 0.25 via `cfgFmtHours`, so Derive's own output is contradicted by the display and by what gets saved. Meets Derive INCONSISTENT criteria (a) aggregate diverges AND (b) modal view ≠ grid view for the same month. **Severity note (vs. F2-3):** both F2-2 and F2-3 diverge only in the `planning` forecast field (soldHours untouched in both), but F2-2 is rated higher because it is a genuine two-step self-contradiction — Derive computes and displays one total in its own confirmation modal, then a second, different rounding pass silently overwrites it before save, so the tool disagrees with itself within a single user action. F2-3's drift is a single-step rounding artifact of redistribution math, not a tool contradicting its own immediately-prior statement.
...
- **Description:** Two sibling operations that populate the same grid disagree on rounding granularity, and only one matches the display layer — drift that was never unified. **Related observation:** the 0.25 display grid itself is only a partial fit for the confirmed sold-hours set {0, 0.25, 0.4, 0.75} — `roundToQuarterHour(0.4) = Math.round(1.6)/4 = 0.5`, so even a single-month task with no redistribution at all would see its 0.4h sold value visually snapped to 0.5h if ever displayed through this grid. This is implicit in F2-3's drift mechanism but worth calling out explicitly: the display grid's resolution (quarter-hour) doesn't fully cover the allowed input set (which includes 0.4, not a quarter-hour multiple).
...
- **Involves:** F1-1 (timesheets.js DD/MM/YYYY assumption); Reforecast's past/future bucketing mechanism (Domain 2 narrative, `config-form.js:824-836` — not F2-3 itself, which is the separate *redistribution rounding* finding downstream of correct bucketing)
