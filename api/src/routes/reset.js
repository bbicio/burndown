...
// Keeps resource_shares in sync with cost_grids.owner_id: the previous owner's 'owner' row is
// removed (they keep no special access unless separately re-shared) and the new owner gets an
// 'owner' row via upsert (in case they already held some other permission on this cost grid).
// Without this, resource_shares — which the share modal and inline share list actually read —
// kept pointing at the old owner forever, blocking them from being re-shared and never showing
// the real new owner in "who has access".
router.patch('/cost-grid/:cgId/owner', async (req, res, next) => {
  ...
  try {
    ...
    const user = await client.query(`SELECT id, first_name, last_name FROM users WHERE id = $1 AND status = 'active'`, [ownerId]);
