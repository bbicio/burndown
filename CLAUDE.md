...
js/lib/                  — pure functions extracted for unit testing (vitest + jsdom), each an ES module
                            ...
                            *in addition to* this export, since it's called directly from template expressions,
                            ...
                            `notif-browser.js` (2026-09) — `shouldShowBrowserNotification(permission, isPageVisible)`:
                            `permission === 'granted' && !isPageVisible`, the sole decision logic behind whether a
                            native browser `Notification` fires for an incoming SSE push (see `js/notifications.js`'s
                            own entry below for the full feature). Deliberately trivial and pure so it's the one part
                            of the feature covered by `vitest` — the rest (permission prompts, the actual `Notification`
                            constructor, focus/click handling) is DOM/browser-API surface, verified manually instead.
                            Loaded via `<script type="module">` on every one of the 10 authenticated pages that also
...
js/notifications.js      — bell icon + SSE notification panel; initNotifications(user) called by nav.js. **Browser/desktop notifications (2026-09)**: a client-side-only, best-effort echo of the same SSE "push" delivery — no server change, no new API. `wireBrowserNotifBanner()` shows the `#nav-notif-browser-banner` markup (injected by `js/nav.js`, see that file's entry) only when `Notification.permission === 'default'`; clicking its "Enable" button calls `Notification.requestPermission()` and hides the banner regardless of the outcome (the browser itself remembers grant/deny per-origin, no app-side persistence needed). `maybeShowBrowserNotification(n)`, called from `openSseStream()`'s message handler alongside the existing `prependNotification(n)`, shows a native `Notification` only when `js/lib/notif-browser.js`'s pure `shouldShowBrowserNotification(permission, isPageVisible)` returns true — permission granted **and** the tab isn't currently focused/visible (`document.hasFocus() && !document.hidden`), avoiding a redundant popup when the user is already looking at the panel. Clicking the popup focuses the tab and follows `n.url` if present, mirroring the panel item's own click behavior. Every check is guarded by `typeof Notification === 'undefined'` for unsupported browsers/contexts. This file itself had never carried a cache-bust `?v=N` on any of its 10 loading pages before this change — added `?v=1` now that its content is no longer static (see the Cache-busting section above).
