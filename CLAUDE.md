...
admin.html               — user management (invite, role, disable, anonymize; admin **or sysadmin**). T&C editor removed (2026-09, moved to `_terms-editor.html`, sysadmin-exclusive). Role model is now 3-tier (`user` < `admin` < `sysadmin`, `018_sysadmin_role.sql`) — sysadmin inherits every admin capability plus two exclusives carved out of the plain admin tier: `_db-reset.html` and Terms & Conditions editing. Full implementation narrative (role toggle/grant-sysadmin gating, canModifyUser guard, Resend invite): see [docs/pages/admin.md](docs/pages/admin.md).
_terms-editor.html       — sysadmin-exclusive (`role !== 'sysadmin'` gate, not admin-or-sysadmin) Terms & Conditions console, same pattern as `_db-reset.html`; reachable only from the sysadmin-only navbar menu block (`js/nav.js`, tab id `termseditor`). Full implementation narrative (base state, Version history): see [docs/pages/terms-editor.md](docs/pages/terms-editor.md).
_db-reset.html           — now sysadmin-exclusive (was admin-only; narrowed 2026-09 as one of the two privileges carved out of the plain admin tier alongside Terms & Conditions editing), gate `role !== 'sysadmin'`, `initNav('dbreset', ...)`; unchanged otherwise (bulk-delete-by-scope grid, "Delete single proposal", "Change proposal owner" widgets — see Pages table above for widget detail). Full narrative: [docs/pages/db-reset.md](docs/pages/db-reset.md).
terms.html               — standalone T&C acceptance page (no navbar/initNav), Vue 3 (CDN, no build step, same pattern as login.html); redirected to by initNav() gate. Full narrative: [docs/pages/terms.md](docs/pages/terms.md).
