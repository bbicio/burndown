...
- **Two-column body** (a `d-flex flex-grow-1`, no separate `#pbDetailContent` wrapper — direct child of `#pbDetailPanel`):
  - Left column (50%): offer metadata (owner name + version creation date) + inline share list (`<share-list>`, see "Sharing" below) + linked projects, `overflow-y:auto`, `border-right`
