...
| `_terms-editor.html` | `/_terms-editor.html` | **Sysadmin-exclusive** hidden page — Terms & Conditions editor (view current version, edit HTML content, save draft / publish new version), Vue 3 (CDN, no build step, same pattern as `_db-reset.html`), linked from the sysadmin-only navbar menu (`initNav('termseditor', ...)`); moved here from `admin.html`'s former T&C card, which is now removed |
...
_terms-editor.html       — sysadmin-exclusive (`role !== 'sysadmin'` gate, not admin-or-sysadmin) Terms & Conditions console (2026-09), same pattern as `_db-reset.html`; hosts exactly the card moved out of `admin.html` (same `terms` state shape, `loadTerms()`/`saveTerms()`/`fmtDate()`) — `loadTerms()` wraps its fetch in try/catch (a fix-wave addition, reusing `terms.msg`/`msgOk` so a failed GET shows a visible error instead of an infinite loading spinner, unlike the pattern it was copied from before this fix). Reachable only from the sysadmin-only navbar menu block (`js/nav.js`, tab id `termseditor`).
...
api/src/routes/app-settings.js   — GET /api/app-settings/terms (requireAuth); PUT /api/app-settings/terms (**sysadmin-exclusive**,
                                    2026-09 — was `requireAdmin`, now `requireSysAdmin`; editing UI moved from
                                    `admin.html` to `_terms-editor.html`); publishNewVersion=true increments
                                    terms_version forcing all users to re-accept
