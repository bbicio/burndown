...
Rows: resources (roles, projects, or owners depending on grouping). **By Owner** groups its rows in three levels — **Owner → Project → Task** — aggregating hours across every role assigned to a task into one row per task, attributed to the owner regardless of which role they logged time under (a task with multiple sold roles sums all roles' Sold/Actuals into that single task row). This is deliberate: the view's purpose is to show which task an owner is actually working on, not their role (already known once the person is identified) — role-level detail remains available in the By Project view (Project → Task → Role → Owner).  
